...
class Stars():
    ...
    def generate_stars(self, d_lum=None, extinction=None):
        ...
        self.M_init = gen_star_masses(np.sum(self.gen_n_stars), imf=self.gen_imf_param[indices])
        return
    ...
    def absolute_magnitudes(self, filters=None, realistic_remnants=True, performance_mode=True):
        ...
        if performance_mode and (self._absolute_magnitudes is not None):
        ...
        if performance_mode and (self._absolute_magnitudes is None):
            ...
            if not np.all(name in filters for name in self.mag_names):
    ...
    def spectral_types(self, realistic_remnants=True, performance_mode=True):
        ...
        performance_mode: save the data returned by this function in memory for faster access later.
        """
        if performance_mode and (self._spectral_types is not None):
    ...
    def compact_parameters(self, mag_limit=None, d_lum=None, extinction=None):
        ...
        if (self.compact_mode == 'mag'):
            mass_limit = magnitude_limited(self.n_stars, self.isoc, mag_lim=mag_limit, distance=d_lum,
                                           ext=extinction, filter_name='Ks')
        else:
            mass_limit = number_limited(self.n_stars, self.isoc, imf=self.imf_param)

        # don't increase the upper limit!
        condition = (mass_limit[:, 1] > self.imf_param[:, 1])
        mass_limit[condition, 1] = self.imf_param[condition, 1]

        fraction_generated = form.mass_fraction_from_limits(mass_limit, imf=self.imf_param)
...
def number_limited(n_stars, isoc, imf=None):
    """Retrieves the lower mass limit for which the number of stars does not exceed 10**7.
    Gives back an array of the mass limit(s) (per population)
    The number of stars and corresponding isochrone object are needed,
        imf parameters can also be supplied.
    """
    n_pop = len(n_stars)
    if imf is None:
        imf = np.full([n_pop, len(default_imf_par)], default_imf_par)
    else:
        imf = np.atleast_2d(imf)
    max_masses = isoc.max_isoc_masses()

    # fraction of the total number of stars to generate
    fraction = np.clip(limiting_number / n_stars, 0, 1)
    min_masses = form.mass_limit_from_fraction(fraction, M_max=max_masses, imf=imf)
    mass_limits = np.column_stack([min_masses, max_masses])
    return mass_limits


def magnitude_limited(n_stars, isoc, mag_lim=default_mag_lim, distance=10, ext=0, filter_name='Ks'):
    ...
    iso_M_ini = isoc.get_columns('M_initial')
    magnitudes = isoc.get_columns(filter_name)
    ...
    mag_mask = (magnitudes < abs_mag_lim + 0.1)
    if not mag_mask.any():
        # if limit too high (too low in mag) then it will break
        mag_mask = (magnitudes == np.min(magnitudes))
        warnings.warn(f'Compacting failed, distance ({distance} pc) too large and ' +
                      f'magnitude limit ({abs_mag_lim}) too low.', RuntimeWarning)

    # now take the lowest mass where mask==True and the highest mass in the isochrone
    min_masses = np.zeros(n_pop)
    for i in range(n_pop):
        isoc_mask = isoc.isochrone_mask(i, age=True)
        min_masses[i] = np.min(iso_M_ini[mag_mask][isoc_mask[mag_mask]])
    max_masses = isoc.max_isoc_masses()
    mass_limits = np.column_stack([min_masses, max_masses])
    return mass_limits
