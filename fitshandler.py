...
'''This module contains the interactions with fits files. It is likely biased towards the purpose of the main program.
...
def PrintCard(filename, keyword, index=0, card_index=None):
    '''Prints card: keyword (str or int). Optional arg: (HDUlist) index, card_index.'''
    with fits.open(os.path.join('images', filename)) as hdul:
        if (card_index == None):
            crd = str.upper(keyword) + ' = ' + str(hdul[index].header[keyword]) + '       / ' + hdul[index].header.comments[keyword]
        else:
...
def ChangeHdr(filename, keyword, value, comment='', index=0):
    '''Adds/updates card 'keyword' (str) in the current file. Input: 'value' (str or number) and optionally 'comment' (str). Optional arg: (HDUlist) index.'''
...
def ChangeData(filename, input_data, index=0):
    '''Changes (and saves) the data in the current fits file. Optional arg: (HDUlist) index.'''
...
def GetCardValue(filename, keyword, index=0):
    '''Returns the value of card 'keyword' (str). Returns 0 if value is a string. Optional arg: (HDUlist) index.'''
    with fits.open(os.path.join('images', filename)) as hdul:
...
def NewHdr(keywords, values, comments=''):
    '''Returns a new header object. Inputs are lists of the keywords (str), values (str or number) and optional comments (str)'''
    hdr = fits.Header()
...
def NewFits(filename, input_data, input_header=None):
    '''Saves the input_data to a new file 'filename'. Optional arg: input_header (header object)'''
...
def AddToFits(filename, input_data, input_header=None):
    '''Appends the header/data to fits file if 'filename' exists, creates one if not. Optional arg: input_header (header object)'''
