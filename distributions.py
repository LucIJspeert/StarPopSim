...
def LogNorm(n=1, mean=0.0, s=1.0):
    ...
    return 10.0**np.random.normal(mean, s, n)                                                       #  base 10! (numpy gives 'e**np.random.normal')
    
def LnNorm(n=1, mean=0.0, s=1.0):
    '''Natural-log-normal distribution with some mean and width (s=sigma). Draws n numbers.'''
    return np.random.lognormal(mean, s, n)                                                          # turns out 10.0**np.random.normal is a tad bit slower. Note: these are a different base! (numpy gives e**np.random.normal)
    
def Exponential(n=1, s=1.0):
    '''Exponential distribution with scale height s. Draws n numbers.'''
    return np.random.exponential(s, n)                                                              # self made function -s*np.log(1-np.random.rand(int(n))) is a bit slower

def DoubleExp(n=1, s=1.0, origin=0.0):
    '''Double sided exponential distribution with scale height s. Draws n numbers around origin.'''
